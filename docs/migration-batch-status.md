...
- **Batch 11 — Fleet workspace orchestration:** migrated. The donor's centralized Fleet workspace contract was adapted into `domains/fleet/workspace.js`, composing the canonical Fleet operations, metric, Maps, and live-network services. `AppContext` now exposes `services.fleetWorkspace`, and Fleet Intelligence consumes that canonical orchestration instead of independently coordinating dashboard/opportunity calls. The live/location signal loop and Fleet workspace sections remain canonicalized without importing the donor workspace service.
