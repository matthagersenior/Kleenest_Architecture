-- Every successful owner CRUD mutation is recorded in the same transaction as the mutation.
-- This keeps the Owner Audit surface authoritative without client-side audit writes.
...
begin
  ...
  if v_action='delete' then
    ...
    v_sql:=format('select to_jsonb(x) from (select * from public.%I where id=$1 limit 1) x',v_table); execute v_sql into v_before using p_id;
    if v_before is null then raise exception 'record not found'; end if;
    v_sql:=format('delete from public.%I where id=$1 returning to_jsonb(%I.*)',v_table,v_table); execute v_sql into v_result using p_id;
    v_event_action:='delete';
  elsif v_action='create' then
    select string_agg(format('%I',key),', ' order by key) into v_cols from jsonb_object_keys(p_payload) key where key not in ('id','created_at','updated_at','deleted_at');
    if v_cols is null then raise exception 'no editable fields supplied'; end if;
    v_sql:=format('insert into public.%I (%s) select %s from jsonb_populate_record(null::public.%I,$1) returning to_jsonb(%I.*)',v_table,v_cols,v_cols,v_table,v_table); execute v_sql into v_result using p_payload;
    v_event_action:='create';
  else
    if p_id is null then raise exception 'record id required'; end if;
    select string_agg(format('%I = r.%I',key,key),', ' order by key) into v_sets from jsonb_object_keys(p_payload) key where key not in ('id','created_at','updated_at','deleted_at');
    if v_sets is null then raise exception 'no editable fields supplied'; end if;
    v_sql:=format('select to_jsonb(x) from (select * from public.%I where id=$1 limit 1) x',v_table); execute v_sql into v_before using p_id;
    if v_before is null then raise exception 'record not found'; end if;
    v_sql:=format('update public.%I t set %s from jsonb_populate_record(null::public.%I,$1) r where t.id=$2 returning to_jsonb(t.*)',v_table,v_sets,v_table); execute v_sql into v_result using p_payload,p_id;
    v_event_action:='update';
  end if;
  if v_result is null then raise exception 'mutation returned no record'; end if;
  insert into public.activity_events(event_type,action,actor_id,target_user_id,status,created_at)
  values('owner_crud',v_event_action||':'||v_table,auth.uid(),case when v_table='profiles' then coalesce((v_result->>'id')::uuid,null) else null end,'success',now());
  return v_result;
end;
$$;
