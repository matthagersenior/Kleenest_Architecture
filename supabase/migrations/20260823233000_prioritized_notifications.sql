...
declare v_id uuid; v_target uuid; v_priority text:=case when lower(p_priority) in ('urgent','high','normal','low') then lower(p_priority) else 'normal' end;
begin
 ...
 insert into public.notifications(user_id,title,message,priority,metadata,created_at) values(v_target,p_title,p_message,v_priority,coalesce(p_metadata,'{}'::jsonb)||jsonb_build_object('scope',p_scope,'business_id',p_business_id,'authorized_sender',auth.uid())) returning id into v_id;
 return v_id;
end; $$;
