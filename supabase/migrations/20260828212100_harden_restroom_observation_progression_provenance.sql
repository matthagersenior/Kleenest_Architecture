-- Harden the canonical restroom observation contract.
-- The observation remains a distinct evidence record; progression is awarded at most once
-- for a qualifying verified visit, not once per repeated stationary submission.

create unique index if not exists progression_metric_events_user_metric_idempotency_uidx
  on public.progression_metric_events (user_id, metric, ((metadata->>'idempotency_key')))
