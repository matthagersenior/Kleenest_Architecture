-- Map discovery reconciliation: make brand/category semantics match the canonical UI.
-- Keeps nearby RPC useful for branded operators (e.g. Taco Bell/Walmart) while
-- preserving bathroom/restaurant/cafe/gas/shopping category behavior.
...
with base as (
  ...
  where l.is_active=true
    ...
    and (
      ...
      or (lower(p_category)='restroom' and (l.has_bathroom=true or l.has_restroom=true or lower(coalesce(l.bathroom_verification_status,''))='has_bathroom'))
      ...
      or (lower(p_category)='government' and (l.is_government=true or lower(coalesce(p.category,l.place_type,'')) like '%government%'))
    )
    and (
...
select
  r.id,r.place_id,coalesce(r.place_name,r.name),
  case when r.brand_name is not null and lower(coalesce(r.p_category,r.place_type,''))='service' then 'brand' else coalesce(r.p_category,r.place_type,'service') end,
