-- Fix canonical scheduled ingestion authorization.
-- SECURITY DEFINER functions execute as their owner (postgres), so the
-- ingestion authority check must permit both trusted service_role and the
-- postgres owner used by the privileged function.
