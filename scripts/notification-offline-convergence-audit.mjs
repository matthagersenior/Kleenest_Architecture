...
const required=[
 ...
 ['domains/offline/packs.js',['create_offline_pack','offline_pack_events','findAuthoritativeReplay','shouldAttemptReplay','offline_replayed','client_event_id','fleet.route_stop_timing','fleet_replay_route_stop_timing','prepareFleetRoute','queueFleetRouteStopTiming']],
 ['domains/offline/replay.js',['MAX_OFFLINE_ATTEMPTS','findAuthoritativeReplay','shouldAttemptReplay']],
 ['runtime/FleetRoutePerformanceCard.jsx',['Prepare offline timing','queueFleetRouteStopTiming','Offline timing','Dispatch requires connectivity','occurredAt']],
...
const migration=path.resolve('supabase/migrations/20260830002700_offline_fleet_timing_replay_v1.sql');
if(!fs.existsSync(migration))missing.push('20260830002700_offline_fleet_timing_replay_v1.sql: missing');
else{const text=fs.readFileSync(migration,'utf8');for(const token of ["add column if not exists metadata jsonb","fleet_replay_route_stop_timing","security invoker","client_event_id is required","fleet_record_route_stop_timing","authoritative_replay","revoke execute","grant execute"])if(!text.includes(token))missing.push(`20260830002700_offline_fleet_timing_replay_v1.sql: missing ${token}`)}
...
console.log('Notification → Push → Realtime → Offline convergence audit passed: device registration, authoritative delivery status, authenticated inbox, realtime subscription, bounded/idempotent replay, Fleet stop timing with original occurrence timestamps, and recovery refresh are wired.');
