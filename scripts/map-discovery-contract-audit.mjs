...
const surface = texts.get('runtime/MapSurfaceProduction.jsx') || '';
for (const forbidden of ['demoLocations', 'sampleLocations', 'mockLocations', 'demo accounts', 'fakeLocations']) {
  if (surface.toLowerCase().includes(forbidden.toLowerCase())) missing.push(`runtime/MapSurfaceProduction.jsx: forbidden demo fallback ${forbidden}`);
}
...
// The production renderer sends the canonical amenity filter as an object
// (`amenities: { [amenity]: true }`); the map network service converts that
// object to amenityNames before passing p_amenity_names to the backend RPC
// and discovery function. Accept the concrete production contract here.
...
const nav = fs.readFileSync(path.join(root, 'domain/workspaces.js'), 'utf8');
if (!nav.includes("id:'social',label:'Social',path:'/social'")) missing.push('domain/workspaces.js: social navigation entry missing');
if (missing.length) { console.error(missing.join('\n')); process.exit(1); }
console.log('Canonical Map discovery audit passed: production renderer, amenity-first discovery, bounded OSM/Overpass ingestion, direct live-candidate convergence, visible legend/markers, individual business results, Game Center continuity, social navigation, non-demo fallback, and single discovery ownership are present.');
