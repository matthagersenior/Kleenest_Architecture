...
const required=[
 ...
 ['src/domains/billing/catalog.js',["interval === 'once'",'max_users','PRODUCT_TYPES.ONE_TIME','PRODUCT_TYPES.RECURRING']],
 ...
 ['src/runtime/PricingPage.jsx',['useSearchParams','oneTime','Buy ${card.copy.title}','refreshAuthContext','plan_code']],
 ...
 ['supabase/functions/stripe-billing-webhook/index.ts',['fulfill_single_use_access_checkout','payment_status','single_use_access','checkout.session.completed','fulfillMembership','membership_one_time','plan_code','stripe_payment']]
];
const missing=[];
...
console.log('Access → Commerce convergence audit passed: free access claims remain authoritative, paid access offers and one-time consumer memberships use Stripe payment-mode fulfillment, recurring catalog plans use subscription mode, and all billing state converges through Supabase authority.');
