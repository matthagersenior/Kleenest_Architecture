...
const required=[
 ...
 ['src/runtime/PricingPage.jsx',['useSearchParams','oneTime','Buy ${card.copy.title}','refreshAuthContext','consumerRank','Included in current membership','recurringCode','syncDelays','checkoutConfirmed','session_id','kleenest:billing-updated','portalStateKey','billingSignature',"portal')!=='return"]],
 ['src/runtime/AccessOffersPage.jsx',['priceCents','Checkout with Stripe','services.accessOffers.checkout','services.accessOffers.purchase','services.accessOffers.redeem','checkout=success','syncDelays','session_id','offer=','kleenest:access-offer-updated']],
 ...
 ['supabase/functions/stripe-customer-portal/index.ts',['billingPortal.sessions.create','stripe_billing_customers','portal=return','return_url']],
...
if(!page.includes("rows.some(row=>String(row?.offer_id||'')===String(offerId)"))missing.push('AccessOffersPage.jsx: paid offer return does not confirm the exact fulfilled offer');
if(!page.includes('syncDelays.length-1'))missing.push('AccessOffersPage.jsx: paid offer webhook convergence retry boundary missing');
...
if(!pricing.includes('window.sessionStorage.setItem(portalStateKey,billingSignature'))missing.push('PricingPage.jsx: portal launch does not checkpoint billing state');
if(!pricing.includes("searchParams.get('portal')!=='return'"))missing.push('PricingPage.jsx: portal return convergence is missing');
...
console.log('Access → Commerce convergence audit passed: paid checkout, membership fulfillment, access-offer fulfillment, recurring entitlement recomputation, and customer-portal returns all converge through Stripe-backed Supabase authority before the UI presents updated access.');
