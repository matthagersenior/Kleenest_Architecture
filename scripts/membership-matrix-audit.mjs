...
for (const [groupName, tiers] of Object.entries(PRODUCT_TIERS)) {
  for (const [tierName, tier] of Object.entries(tiers)) {
    ...
    for (const capability of tier.capabilities) {
      ...
      assert(tier.workspaces.some((workspace) => ui.includes('all') || ui.includes(workspace)), `${tier.id}: enabled capability '${capability}' maps to '${domain}', but none of its workspaces expose that capability domain`);
    }
