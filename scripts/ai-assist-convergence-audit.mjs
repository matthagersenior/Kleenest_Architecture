...
const required=[
 ...
 ['src/runtime/RouteSurface.jsx',['AiAssistPanel','task="route_plan"','Route recommendation','freshness and route facts','maximum 80 words','Never ask the user to provide data that is absent','phone:s.phone','openingHours:s.openingHours','amenities:Array.isArray(s.amenities)','freshnessScore:s.freshnessScore','stalenessStatus:s.stalenessStatus','sourceCapturedAt:s.sourceCapturedAt']],
...
console.log('AI Assist convergence audit passed: authenticated free-first OpenRouter -> Gemini -> OpenAI failover, deterministic grounded fallback, server-only provider credentials, normalized safe runtime diagnostics, failure request-ID telemetry, provider attempt observability, provider-state event propagation, centralized service authority, rich task-scoped Route/Fleet/Business/Enterprise/Admin/consumer AI surfaces, and human review before mutations.');
