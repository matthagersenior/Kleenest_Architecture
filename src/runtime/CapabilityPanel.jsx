...
export default function CapabilityPanel({ title, description, load, action, actionLabel = 'Refresh', renderData }) {
  ...
  return <section className="panel capability-panel"><div className="panel-heading"><div><h2>{title}</h2>{description && <p>{description}</p>}</div><button className="button secondary" disabled={busy} onClick={run}>{busy ? 'Loading…' : 'Refresh'}</button></div>{error && <div className="state error">{error.message || 'Unable to complete this operation.'}</div>}{!error && data !== null && (renderData ? renderData(data) : <pre className="data-preview">{JSON.stringify(data, null, 2)}</pre>)}{action && <button className="button primary" disabled={busy} onClick={runAction}>{busy ? 'Working…' : actionLabel}</button>}</section>;
}
...
export function ActionForm({ title, fields, submitLabel = 'Save', onSubmit }) {
  ...
  return <form className="action-form" onSubmit={submit}><h3>{title}</h3>{fields.map(field => <label key={field.name}>{field.label}<input required={field.required !== false} type={field.type || 'text'} value={values[field.name]} onChange={e => setValues(v => ({ ...v, [field.name]: e.target.value }))} placeholder={field.placeholder || ''} /></label>)}{error && <div className="state error">{error}</div>}{message && <div className="state success">{message}</div>}<button className="button primary" disabled={busy}>{busy ? 'Saving…' : submitLabel}</button></form>;
}
