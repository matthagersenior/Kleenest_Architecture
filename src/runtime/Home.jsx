...
export default function Home(){
  ...
  return <WorkspaceShell workspace="consumer">
    <main className="page home-page">
      ...
      <section className="home-section"><div className="section-heading"><div><span className="eyebrow">YOUR NETWORK</span><h2>Stay connected</h2></div></div><div className="home-grid">{[
        ['/community',MessageCircle,'Community','Reviews, discoveries, follows, media, and local activity.'],
        ['/activity',RouteIcon,'Your activity','See what you verified, reviewed, earned, and contributed.'],
        ['/profile',CircleUserRound,'Your profile','Manage identity, progress, preferences, and membership.']
      ].map(([to,Icon,title,text])=><Link className="home-card" to={to} key={to}><span className="home-card-icon"><Icon size={21}/></span><div><h3>{title}</h3><p>{text}</p></div><ArrowRight className="home-card-arrow" size={17}/></Link>)}</div></section>
