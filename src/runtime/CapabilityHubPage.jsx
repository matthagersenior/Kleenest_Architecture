...
const groups = [
  { title: 'Bathroom interaction', items: [['Map','/map','Discover canonical locations, map intelligence and live availability.'],['Routing + offline','/route','Request canonical directions, cache route context and replay offline events.'],['Check-in','/activity','Record visit activity through the canonical service layer.'],['Review + evidence','/evidence','Contribute ratings, observations, photos and provenance.'],['Engagement','/engage','Connect verified activity to progression and rewards.']] },
  ...
  { title: 'Platform intelligence', items: [['Cross-tier intelligence','/intelligence','Shared network signals and leaderboards.'],['Notifications','/notifications','Delivery, priority and preference controls.'],['Admin','/admin','Platform command, integrity and governance.'],['Owner','/owner','Owner controls, tier previews and audit surfaces.']] },
];
...
export default function CapabilityHubPage() {
  ...
  return (
    <WorkspaceShell workspace="consumer">
      ...
      <section className="panel"><strong>Runtime</strong><p>{configured ? 'Supabase configured' : 'Supabase not configured'} · tier: {membershipTier || 'consumer'}</p><div className="action-grid"><Link className="btn primary" to="/integration">Open Full Wiring Hub</Link><Link className="btn" to="/consumer/actions">Open Bathroom Action Center</Link><Link className="btn" to="/route">Open Routing + Offline</Link><Link className="btn" to="/engagement/orchestrate">Open QR + Geofence + Quest</Link></div><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter capabilities…" /></section>
      {groups.map((group) => <section className="section" key={group.title}><h2>{group.title}</h2><div className="card-grid">{group.items.map(([label, href, description]) => <Link className="capability-card" to={href} key={label}><strong>{label}</strong><span>{description}</span></Link>)}</div></section>)}
