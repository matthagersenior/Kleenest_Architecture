...
const groups = [
  ...
  { title: 'Engagement primitives', items: [['QR','/engage','Use QR as an attribution and action trigger.'],['Geofence','/engage','Use proximity events as contextual triggers.'],['Quest','/play','Run predefined routes and tasks.'],['Leaderboards','/intelligence','Turn measurable participation into recognition and rewards.']] },
...
export default function CapabilityHubPage() {
  ...
  return (
    <WorkspaceShell workspace="consumer">
      ...
      <section className="panel"><strong>Runtime</strong><p>{configured ? 'Supabase configured' : 'Supabase not configured'} · tier: {membershipTier || 'consumer'}</p><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Filter capabilities…" /></section>
