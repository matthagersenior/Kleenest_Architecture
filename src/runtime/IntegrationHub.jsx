...
export default function IntegrationHub(){return <WorkspaceShell workspace="owner"><section className="page"><div className="page-header"><div><span className="eyebrow">PLATFORM WIRING</span><h1>Integration map</h1><p>Cross-tier navigation into the canonical product surfaces. Product workflows live in their domains; this page only maps the runtime graph.</p></div></div><div className="detail-grid">{surfaces.map(([name,path,flow,Icon])=><Link className="detail-panel" key={path} to={path}><div className="panel-heading"><div><span className="eyebrow">CANONICAL SURFACE</span><h2>{name}</h2></div><Icon size={22}/></div><p>{flow}</p><span className="secondary">Open surface</span></Link>)}</div><div className="detail-panel"><div className="panel-heading"><div><span className="eyebrow">ARCHITECTURE RULE</span><h2>One product surface per domain</h2></div><ShieldCheck size={22}/></div><p>IntegrationHub does not embed ConsumerActionCenter, EngagementOrchestrator, or duplicate domain workflows. Those capabilities are reached through their canonical routes.</p></div></section></WorkspaceShell>}
