...
export default function RouteSurface() {
  ...
  const addLocationId = params.get('addStop') || '';
  const draft=readDraft();
  const incomingIds=[initialLocationId,addLocationId].filter(Boolean);
  const draftStops=Array.isArray(draft?.stops)?draft.stops.filter(Boolean):[];
  const mergedInitialStops=[...new Set([...draftStops,...incomingIds])];
  ...
  const [stops, setStops] = useState(mergedInitialStops);
  const [stopNames, setStopNames] = useState(draft?.stopNames && typeof draft.stopNames==='object'?draft.stopNames:{});
  const [route, setRoute] = useState(null);
  const [status, setStatus] = useState(incomingIds.length ? 'Restroom stop added to your route. Add more stops or build your route.' : draftStops.length ? 'Route draft restored. Add more stops or build your route.' : 'Start with where you are going, or choose restroom stops from Explore.');
  const [busy, setBusy] = useState('');

  useEffect(()=>{writeDraft({origin,destination,stops,stopNames});},[origin,destination,stops,stopNames]);

  useEffect(() => {
    const ids=[...new Set([...incomingIds])];
    if (!ids.length || !services?.locations) return undefined;
    let active=true;
    Promise.all(ids.map(id=>services.locations.getById(id).catch(()=>null))).then(rows=>{
      if(!active)return;
      const additions={};
      rows.forEach((place,i)=>{if(place){const id=ids[i];additions[id]={name:place.name||'Restroom stop',address:[place.address,place.city,place.state].filter(Boolean).join(', ')}}});
      if(Object.keys(additions).length)setStopNames(v=>({...v,...additions}));
    });
    return()=>{active=false;};
  },[initialLocationId,addLocationId,services]);

  useEffect(() => {
    if (origin || !navigator.geolocation) return undefined;
    let active = true;
    navigator.geolocation.getCurrentPosition(({ coords }) => active && setOrigin(`${coords.latitude},${coords.longitude}`), () => {}, { enableHighAccuracy: true, timeout: 10000, maximumAge: 120000 });
    return () => { active = false; };
  }, [origin]);
  ...
  const useGps = () => {
    ...
    setBusy('gps');
    navigator.geolocation.getCurrentPosition(({ coords }) => { setOrigin(`${coords.latitude},${coords.longitude}`); setStatus('Starting point updated.'); setBusy(''); }, () => { setStatus('Location permission was unavailable. Enter an address instead.'); setBusy(''); }, { enableHighAccuracy: true, timeout: 12000, maximumAge: 120000 });
  };

  const addStop = async () => {
    const value = locationId.trim();
    if (!value) return setStatus('Enter or select a location before adding a stop.');
    if (stops.includes(value)) return setStatus('That restroom is already in your route.');
    setStops(v => [...v, value]); setLocationId(''); setRoute(null); setStatus(`Restroom stop ${stops.length + 1} added.`);
    if (services?.locations?.getById) { try { const place=await services.locations.getById(value); if(place)setStopNames(v=>({...v,[value]:{name:place.name||'Restroom stop',address:[place.address,place.city,place.state].filter(Boolean).join(', ')}})); } catch (_) {} }
  };
  ...
  const clearDraft=()=>{try{localStorage.removeItem(DRAFT_KEY);}catch{}setStops([]);setStopNames({});setRoute(null);setStatus('Route stops cleared.');};

  const request = async () => {
    setBusy('route');
    try {
      const selectedStops=[...stops];
      setStatus(selectedStops.length?`Building the route through ${selectedStops.length} restroom stop${selectedStops.length===1?'':'s'}…`:'Building your route…');
      ...
      setRoute({...data,stopLocationIds:data?.stopLocationIds||selectedStops}); setStatus('Route ready.');
      window.dispatchEvent(new CustomEvent('kleenest:route-updated',{detail:{locationId:data?.locationId||selectedStops[selectedStops.length-1]||null,route:data,stopLocationIds:selectedStops}}));
    } catch(e){setRoute(null);setStatus(e?.message||'We could not build that route.');} finally{setBusy('');}
  };
  ...
  return <WorkspaceShell workspace="consumer"><main className="page route-page">
    ...
    <section className="detail-panel route-planner">
      <div className="panel-heading route-panel-heading"><div><span className="eyebrow">PLAN</span><h2>Where are you starting and going?</h2></div><div className="route-heading-icon"><Navigation size={20}/></div></div>
      ...
      <div className="route-stop-picker"><div className="route-stop-picker-heading"><div><span className="route-field-title">Restroom stops</span><span className="route-field-help">Add as many stops as you need. Stops selected from the map or location details stay in this route draft.</span></div><Link className="button secondary route-explore" to="/map"><MapPin size={15}/>Explore stops</Link></div>
        <div className="manual-stop-row"><input value={locationId} onChange={e=>setLocationId(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addStop()} placeholder="Paste a location ID" aria-label="Location ID"/><button className="button secondary" onClick={addStop} disabled={!locationId.trim()||busy!==''}><Plus size={15}/>Add stop</button></div>
        ...
        {stops.length>0&&<div className="route-actions"><button className="button secondary" onClick={clearDraft} disabled={busy!==''}><Trash2 size={15}/>Clear stops</button></div>}
      </div>
