export function createProgressionService(client) {
  ...
  return Object.freeze({
    ...
    reviewRewards: async reviewId => { await requireUser(); if (!reviewId) throw new Error('Review is required.'); return rpc('review_rewards_summary', { p_review_id: reviewId }); },
    promotionRewards: async redemptionId => { await requireUser(); if (!redemptionId) throw new Error('Redemption is required.'); return rpc('promotion_redemption_rewards_summary', { p_redemption_id: redemptionId }); },
    checkinRewards: async checkInId => { await requireUser(); if (!checkInId) throw new Error('Check-in is required.'); return rpc('checkin_rewards_summary', { p_checkin_id: checkInId }); },
  });
}
