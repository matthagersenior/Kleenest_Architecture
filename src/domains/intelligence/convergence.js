...
export function createIntelligenceConvergenceService(client,{intelligence,notifications,actions,live}={}){
  ...
  return Object.freeze({
    async notifyFromSignal({locationId,surface='network',type,dedupeKey,title,body,reasons=[],signals={},cooldownMinutes=120}={}){
      ...
      const data=await rpc('create_intelligence_notification',{p_user_id:user.id,p_location_id:locationId,p_surface:surface,p_type:type,p_dedupe_key:dedupeKey,p_title:title,p_body:body,p_data:{reasons,signals,generated_at:new Date().toISOString()},p_cooldown_minutes:cooldownMinutes});emit('kleenest:intelligence-updated',{locationId,type,notification:data});return data;
    },
    async createAction({locationId,businessId,surface,signalType,actionType,metadata={}}={}){if(!actions?.createLink)throw new Error('Intelligence action service is unavailable.');const result=await actions.createLink(locationId,businessId,surface,signalType,actionType,metadata);emit('kleenest:intelligence-updated',{locationId,businessId,signalType,actionType,result});return result;},
    ...
    async publishLocationSignal({locationId,eventType,title,body,payload={},radiusM=500,dedupeKey=null}={}){const data=await rpc('publish_intelligence_location_event',{p_location_id:locationId,p_event_type:eventType,p_title:title,p_body:body,p_payload:payload,p_radius_m:radiusM,p_dedupe_key:dedupeKey});emit('kleenest:intelligence-updated',{locationId,eventType,result:data});return data;},
    async operationalSnapshot({businessId,surface='fleet',start=null,end=null}={}){if(!businessId)throw new Error('Business is required.');const id=String(businessId);if(surface==='fleet'){const[dashboard,opportunities]=await Promise.all([rpc('fleet_dashboard_summary_v2',{p_business_id:id}),rpc('fleet_service_opportunities_for_business',{p_business_id:id})]);return{surface,businessId:id,dashboard:dashboard??null,opportunities:rows(opportunities)};}const[dashboard,locationIntelligence,roi]=await Promise.all([rpc('business_dashboard_secure_summary',{p_business_id:id,p_start:start,p_end:end}),rpc('business_location_intelligence',{p_business_id:id,p_start:start,p_end:end}),rpc('business_roi_analytics',{p_business_id:id,p_start:start,p_end:end})]);return{surface,businessId:id,dashboard:dashboard??null,locationIntelligence:rows(locationIntelligence),roi:roi??null};},
    async operationalOpportunities({businessId,surface='fleet'}={}){if(!businessId)throw new Error('Business is required.');const id=String(businessId);const result=surface==='fleet'?await rpc('fleet_service_opportunities_for_business',{p_business_id:id}):await rpc('business_location_intelligence',{p_business_id:id});return rows(result);},
