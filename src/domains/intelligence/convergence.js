...
export function createIntelligenceConvergenceService(client,{intelligence,notifications,actions,live}={}){
  ...
  return Object.freeze({
    ...
    async operationalOpportunities({businessId,surface='fleet'}={}){if(!businessId)throw new Error('Business is required.');const id=String(businessId);return rows(await rpc(surface==='fleet'?'fleet_service_opportunities_for_business':'business_location_intelligence',{p_business_id:id}));},
    ...
    async processJobs(limit=25){const[notificationsResult,actionsResult]=await Promise.all([client.rpc('process_intelligence_notification_jobs',{p_limit:limit}),client.rpc('process_intelligence_action_jobs',{p_limit:limit})]);if(notificationsResult.error)throw notificationsResult.error;if(actionsResult.error)throw actionsResult.error;return{notifications:notificationsResult.data??0,actions:actionsResult.data??0};},
    recordEvent,
