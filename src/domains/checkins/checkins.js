...
export function createCheckInService(client) {
  ...
  async function publishCheckIn(locationId, payload) {
    if (!locationId) return null;
    try { return await live.publish({ type: LIVE_EVENT_TYPES.QR_CHECK_IN, locationId, payload }); }
    catch { return null; }
  }
  ...
  return Object.freeze({
    byQr: async ({ placeId, qrToken }) => {
      ...
      if (error) throw error;
      await publishCheckIn(placeId, { method: 'qr', result: data });
      return data;
    },
    byGps: async ({ latitude, longitude, radiusMeters = 100, locationId = null }) => {
      await requireUser();
      ...
      await publishCheckIn(resolvedLocationId, { method: 'gps', latitude: Number(latitude), longitude: Number(longitude), result: data });
      return data;
    },
    fromMap: async ({ locationId, latitude, longitude }) => {
      ...
      if (error) throw error;
      await publishCheckIn(locationId, { method: 'map', latitude: Number(latitude), longitude: Number(longitude), result: data });
      return data;
    },
    verifyQr: async ({ qrCode, latitude, longitude }) => {
      ...
      if (error) throw error;
      const resolvedLocationId = data?.location_id || data?.place_id || null;
      await publishCheckIn(resolvedLocationId, { method: 'verified_qr', latitude: Number(latitude), longitude: Number(longitude), result: data });
      return data;
    },
