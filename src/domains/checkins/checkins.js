...
export function createCheckInService(client) {
  ...
  async function completed(method, data, locationId) { emit('checkin-completed', { method, checkIn: data, locationId: locationId || data?.location_id || data?.place_id || null }); emit('progression-updated', { type: 'checkin_completed', method, checkIn: data }); return data; }
