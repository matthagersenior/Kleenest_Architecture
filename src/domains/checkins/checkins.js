...
export function createCheckInService(client,{quests=null}={}) {
  ...
  async function trustRefresh(locationId) { if (!locationId) return null; try { const { data,error }=await client.rpc('refresh_location_trust_state',{p_location_id:locationId}); if(error) throw error; emit('location-trust-refreshed',{locationId,result:data}); return data; } catch { return null; } }
  async function recordVerificationStreak(locationId) { if (!locationId) return null; try { const { data,error }=await client.rpc('record_verification_streak',{p_location_id:locationId}); if(error) throw error; emit('verification-streak-updated',{locationId,result:data}); return data; } catch { return null; } }
  async function reconcileRouteStop(locationId, checkInId) {
    if (!locationId || !checkInId) return null;
    try { const { data,error }=await client.rpc('arrive_active_route_stop',{p_location_id:locationId,p_check_in_id:checkInId}); if(error) throw error; if(data?.matched) emit('route-stop-arrived',{locationId,checkInId,result:data}); return data; } catch { return null; }
  }
  ...
  async function completed(method,data,locationId) {
    ...
    await trustRefresh(resolvedLocationId);
    await recordVerificationStreak(resolvedLocationId);
  ...
  return Object.freeze({
    ...
    leaveLocation,
    getTrustState: async locationId=>{const {data,error}=await client.rpc('get_location_trust_state',{p_location_id:locationId});if(error)throw error;return data;},
    reverificationTargets: async limit=>{const {data,error}=await client.rpc('select_reverification_targets',{p_limit:Number(limit||25)});if(error)throw error;return data;},
    verificationStreak: async()=>{await requireUser();const {data,error}=await client.from('verification_streaks').select('*').single();if(error&&error.code!=='PGRST116')throw error;return data||null;},
