export const LIVE_EVENT_TYPES = Object.freeze({
  ...
  LOCATION_VERIFIED: 'location.verified', LOCATION_STALE: 'location.stale', LOCATION_CONFLICT: 'location.conflict', BUSINESS_OFFER_STARTED: 'business.offer_started',
...
export function createLiveNetworkService(client) {
  ...
  return Object.freeze({
    ...
    list: async ({ locationId = null, types = null, limit = 100 } = {}) => {
      let query = client.from('live_network_events').select('id,event_type,location_id,actor_type,actor_id,payload,created_at').order('created_at', { ascending: false }).limit(limit);
