...
async function uploadWithClient(client, { file, userId, checkInId, locationId }) {
  ...
  const { data, error } = await client.rpc('submit_location_photo_record', {
...
export async function uploadEvidencePhoto(args) {
  return uploadWithClient(supabase, args);
}

export function createConsumerPhotoService(client = supabase) {
  return Object.freeze({ uploadEvidencePhoto: args => uploadWithClient(client, args) });
}
