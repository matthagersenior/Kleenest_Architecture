export function createLocationEvidenceService(client,{quests=null,intelligence=null}={}) {
  ...
  async function dispatch(type,values,result){
    ...
    if(!quests)return;
    try{await quests.dispatchEvent(type,{locationId:values.locationId,checkinId:values.checkInId||null,metadata:{observationType:values.observationType||null,amenityId:values.amenityId||null,observedQuantity:numberOrNull(values.observedQuantity),photoId:values.photoId||null,result,trust,intelligence:intelligenceResult}})}catch{}
  }
  return Object.freeze({
    ...
    amenityObservation: async values => { const metadata={...(values.metadata||{})}; const observedQuantity=numberOrNull(values.observedQuantity); if(observedQuantity!=null)metadata.observed_quantity=observedQuantity; const result=await rpc('submit_amenity_observation', { p_location_id: values.locationId, p_amenity_id: values.amenityId, p_status: values.status, p_confidence: numberOrNull(values.confidence), p_verification_method: values.verificationMethod || null, p_check_in_id: values.checkInId || null, p_photo_id: values.photoId || null, p_notes: values.notes || null, p_metadata: metadata }); await dispatch('amenity_observation',values,result); return result; },
    ...
    trustConflicts:locationId=>rpc('get_location_trust_conflicts',{p_location_id:locationId}),
    ...
    occupancyTrend:(locationId,{hours=24,bucketMinutes=120}={})=>rpc('get_location_occupancy_trend',{p_location_id:locationId,p_hours:Number(hours)||24,p_bucket_minutes:Number(bucketMinutes)||120}),
