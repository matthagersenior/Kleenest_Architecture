export function createLocationEvidenceService(client,{quests=null}={}) {
  ...
  async function dispatch(type,values,result){ if(!quests)return; try{await quests.dispatchEvent(type,{locationId:values.locationId,checkinId:values.checkInId||null,metadata:{observationType:values.observationType||null,amenityId:values.amenityId||null,result}})}catch{} }
