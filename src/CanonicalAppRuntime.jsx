...
import { Activity, BarChart3, Building2, ChevronRight, Compass, Crown, Database, Fleet, Map, Menu, ShieldCheck, Sparkles, Trophy, Users, X } from 'lucide-react';
import { useMemo, useState } from 'react';
import { supabase } from './infrastructure/supabase/client.js';

const WORKSPACES = [
  { id: 'consumer', label: 'Kleenest', path: '/', icon: Compass, tone: 'consumer' },
  { id: 'business', label: 'Business', path: '/business', icon: Building2, tone: 'business' },
  { id: 'fleet', label: 'Fleet', path: '/fleet', icon: Activity, tone: 'fleet' },
  { id: 'enterprise', label: 'Enterprise', path: '/enterprise', icon: BarChart3, tone: 'enterprise' },
  { id: 'admin', label: 'Admin', path: '/admin', icon: ShieldCheck, tone: 'admin' },
];

const MEMBERSHIPS = [
  { id: 'free', label: 'Free', detail: 'Explore Kleenest', ads: true },
  { id: 'premium', label: 'Premium', detail: 'Ad-free for a one-time $5 purchase', ads: false },
  { id: 'family', label: 'Family', detail: 'Shared household experience', ads: false },
  { id: 'business', label: 'Business', detail: 'Growth and engagement', ads: false },
  { id: 'enterprise', label: 'Enterprise', detail: 'Advanced data and partnerships', ads: false },
  { id: 'fleet', label: 'Fleet', detail: 'Operations and network performance', ads: false },
  { id: 'admin', label: 'Admin', detail: 'Full test and control surface', ads: false },
];
    ...
    <div className={`app workspace-${workspace}`}>
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <span className="workspace-label">{current.label}</span>
        </div>
      </header>
      <main>{children}</main>
    </div>
...
function Home() {
  return <Shell workspace="consumer"><section className="hero"><div><span className="eyebrow">Find better. Live better.</span><h1>Your cleanest route through the world.</h1><p>Kleenest connects places, people, routes, rewards, and real-world community intelligence in one simple experience.</p><div className="hero-actions"><Link className="button primary" to="/map">Explore the network <ChevronRight size={17} /></Link><Link className="button secondary" to="/play">Play & earn</Link></div></div><div className="hero-card"><Compass size={28}/><strong>Live network</strong><span>Discover current places, activity, quality signals, and routes.</span></div></section><section className="workspace-grid">{WORKSPACES.slice(1).map(({id,label,icon:Icon,path}) => <Link className="workspace-card" key={id} to={path}><Icon size={22}/><span><strong>{label}</strong><small>{MEMBERSHIPS.find(item => item.id === id)?.detail}</small></span><ChevronRight size={18}/></Link>)}</section></Shell>;
}
...
function WorkspaceLanding({ workspace }) {
  ...
  return <Shell workspace={workspace}><section className="page-head"><span className="eyebrow">{meta.label}</span><h1>{meta.detail}</h1><p>The architecture surface is intentionally capability-first: every card is a destination for a real domain service, not a decorative placeholder.</p></section><section className="feature-grid">{sections.map((label, index) => <Link className="feature-card" key={label} to={`${workspace === 'fleet' ? '/fleet' : `/${workspace}`}${index === 0 ? '' : `/${label.toLowerCase().replaceAll(' ', '-')}`}`}><span className="feature-icon">{index === 0 ? <Sparkles size={20}/> : index === 1 ? <Map size={20}/> : index === 2 ? <BarChart3 size={20}/> : <ChevronRight size={20}/>}</span><strong>{label}</strong><small>Connects to the canonical {label.toLowerCase()} capability.</small></Link>)}</section></Shell>;
}
...
export default function CanonicalAppRuntime() {
  ...
  if (!supabase && location.pathname.startsWith('/admin')) return <SimpleSurface title="Configuration required" detail="Admin surfaces require the configured Supabase boundary." icon={Database}/>;
  return <Routes>
